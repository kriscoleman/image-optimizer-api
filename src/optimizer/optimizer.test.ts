...
jest.mock('../get-image-buffer-from-url/get-image-buffer-from-url');
...
describe('optimizer', () => {
  beforeEach(() => {
    ...
    (getImageBufferFromUrl as jest.Mock).mockImplementation(async () => {
      return Buffer.from(tinyImageDataUrl);
    });
  });
